...
#Banner
echo "======================================================"
echo "+            uninstall battery notifier              +"
echo "======================================================"                                                  
...
#Remove job
echo -n "[*] Removing job from cron ... "
crontab -l | grep -v '~/.crazyScripts/BatteryNotify/main.sh' | crontab -
if [ $? -eq 0 ]; then
	echo "${GREEN}[Done]${NC}"
else
	echo "${RED}[Failed]${NC}"
...
rm -r ~/.crazyScripts/BatteryNotify > /dev/null 2>&1
if [ $? -eq 0 ]; then
	echo "${GREEN}[Done]${NC}"
else
        echo "${RED}[Failed]${NC}"
